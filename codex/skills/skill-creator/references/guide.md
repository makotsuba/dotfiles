...
1. **YAML frontmatter**: Always in system prompt. Minimal — just enough to know WHEN to use the skill.
...
```yaml
license: MIT
allowed-tools: "Bash(python:*) Bash(npm:*) WebFetch"  # Space-separated string
...
- No reserved words in skill name (check Codex docs for current list)
...
- Ask: "When would you use the [skill-name] skill?" to verify trigger behavior
