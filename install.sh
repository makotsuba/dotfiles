...
KIRO_DIR="$HOME/.kiro"
KIRO_STEERING_GLOBAL_DIR="$KIRO_DIR/steering/global"
...
parse_components() {
    case "$#" in
        0)
            fail "--components is required; choose one or more of: claude,codex,kiro,shell"
            return 1
            ;;
        1)
            if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    ...
    for component in "${components[@]}"; do
        ...
        case "$component" in
            ...
            shell)
                ...
                ;;
            all)
                fail 'component "all" is not supported; list component names explicitly'
                return 1
                ;;
            *)
...
preflight_claude_component() {
    ...
    if [ "$OS" = "wsl" ]; then
        ...
        require_command ssh.exe || return 1
...
preflight_kiro_component() {
    ...
    for source in \
        ...
        "$DOTFILES_DIR/kiro/steering/global/aws-infrastructure.md"; do
        require_source_file "$source" || return 1
    done

    # Kiro CLI reads ~/.kiro/steering/**/*.md, so the directories must stay real
    # directories; only the markdown files themselves are symlinked.
    for directory in \
        "$KIRO_DIR" \
        "$KIRO_DIR/steering" \
        "$KIRO_STEERING_GLOBAL_DIR"; do
        validate_managed_directory "$directory" || return 1
    done

    for target in \
        "$KIRO_STEERING_GLOBAL_DIR/default.md" \
        "$KIRO_STEERING_GLOBAL_DIR/aws-infrastructure.md"; do
        validate_managed_target "$target" || return 1
    done
}

install_kiro_component() {
    mkdir -p "$KIRO_STEERING_GLOBAL_DIR"

    force_link "$DOTFILES_DIR/kiro/steering/global/default.md" \
        "$KIRO_STEERING_GLOBAL_DIR/default.md"
    force_link "$DOTFILES_DIR/kiro/steering/global/aws-infrastructure.md" \
        "$KIRO_STEERING_GLOBAL_DIR/aws-infrastructure.md"

    echo "Done! Kiro CLI steering files installed to $KIRO_STEERING_GLOBAL_DIR"
}
...
preflight_selected_components() {
    ...
    if [ "$INSTALL_KIRO" = true ]; then
        preflight_kiro_component || return 1
    fi
    if [ "$INSTALL_SHELL" = true ]; then
